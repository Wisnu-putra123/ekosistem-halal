...
<head>
    ...
    <title>Registrasi - Ekosistem Halal</title>
...
<body class="bg-slate-100 min-h-screen flex items-center justify-center p-4 py-8">
    ...
    <div class="w-full max-w-md bg-white rounded-2xl shadow-lg border border-slate-200/80 p-6 sm:p-8">
        ...
        <div class="text-center mb-6">
            <a href="{{ url('/') }}" class="inline-flex items-center justify-center w-14 h-14 bg-emerald-50 text-emerald-600 rounded-full mb-3 hover:bg-emerald-100 transition">
                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"></path>
                </svg>
            </a>
            <h2 class="text-2xl font-bold text-slate-800">Daftar Akun Baru</h2>
            <p class="text-sm text-slate-500 mt-1">Registrasi untuk mulai menggunakan aplikasi</p>
        </div>
        ...
        <form action="{{ route('register') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="Masukkan nama lengkap" required autofocus
                           class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition">
                </div>
                @error('name')
            ...
            <button type="submit" class="w-full py-2.5 px-4 bg-emerald-600 hover:bg-emerald-700 text-white font-medium text-sm rounded-lg shadow-sm transition duration-150">
                Daftar Sekarang
            </button>

            <div class="text-center pt-2">
                <p class="text-sm text-slate-600">
                    Sudah memiliki akun? <a href="{{ route('login') }}" class="font-semibold text-emerald-600 hover:text-emerald-700">Login ke Akun Anda</a>
                </p>
            </div>
        </form>
    </div>
